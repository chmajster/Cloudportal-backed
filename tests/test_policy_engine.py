...
def test_policy_scope_rejects_unknown_dimensions():
    from pydantic import ValidationError
    from app.policy_engine.schemas import PolicyInput

    with pytest.raises(ValidationError, match='Unsupported scope dimensions'):
        PolicyInput(
            name='typo',
            policy_type='access',
            scope={'enviroments': ['prod']},
            effects=[{'type': 'allow'}],
        )


def test_staged_approval_definition_is_validated():
    from app.policy_engine.engine import validate_effects

    with pytest.raises(ValueError, match='stage must be an object'):
        validate_effects([{
            'type': 'require_approval',
            'stages': ['Security'],
        }])

    with pytest.raises(ValueError, match='requires permission'):
        validate_effects([{
            'type': 'require_approval',
            'stages': [{'name': 'Security', 'approver': {'type': 'permission'}}],
        }])


def test_regex_policy_rejects_backtracking_patterns():
...
def test_policy_api_versions_simulator_and_whitelist(client, headers):
    ...
    assert allowed.json()['matched_policy_versions'] == [{
        'policy_id': item['id'],
        'version': 1,
    }]
