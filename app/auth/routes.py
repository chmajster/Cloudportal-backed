    ...
    if valid and ldap_profile:
        ...
        if valid and user:
            if user.auth_source != 'ldap':
                ...
                username_collision = db.scalar(select(User.id).where(
                    User.username == ldap_username, User.id != user.id
                ))
                email_collision = db.scalar(select(User.id).where(
                    User.email == ldap_email, User.id != user.id
                ))
                external_collision = db.scalar(select(User.id).where(
                    User.external_id == ldap_profile['dn'], User.id != user.id
                ))
