...
(() => {
  ...
  async function openBlueprintWizard(options = {}) {
    if (!parts.core || !parts.hostname || !parts.network || !parts.scope || !parts.ui || !parts.cloudInit || !parts.awx || !parts.validation || !parts.routing) {
    ...
    try {
      ...
      state.step = parts.routing.normalizedStep(options.initialStep || 0, STEPS.length);
      ...
      async function submitBlueprint() {
        ...
        try {
          ...
          parts.routing.replaceRoute(created, state, options, STEPS.length);
      ...
      function render() {
        ...
        parts.routing.syncPage(editingItem, state, options, STEPS.length);
      ...
      footerRoot = parts.routing.mount(editingItem, state, options, shell);
  ...
  window.BlueprintWizard = Object.freeze({
    open: options => parts.routing.navigateWizard(options, STEPS.length),
    render: openBlueprintWizard,
    path: options => parts.routing.path({ ...(options || {}), total: STEPS.length }),
  });
