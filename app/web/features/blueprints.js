...
function blueprintFixedApmid(item) {
...
async function executeBlueprint(item) {
  try {
    ...
    const fixedApmid = blueprintFixedApmid(item);
    const runtimeApmidRequired = item.deployment?.template === 'proxmox-vm' && !fixedApmid;
    ...
    if (runtimeApmidRequired) {
      const classification = await api('/vm-classification/options');
      const apmids = [...new Set((classification.apmids || []).map(value => String(value).trim().toUpperCase()).filter(Boolean))];
      if (!apmids.length) {
      ...
      fields.append(node('div', { class: 'field-help wide', text: 'APMID jest zdefiniowany w Blueprintcie: ' + fixedApmid + '.' }));
    }
    ...
    openModal({
      ...
      onSubmit: async (_data, form) => {
        ...
        if (runtimeApmidRequired) payload.apmid = form.elements.runtime_apmid.value;
