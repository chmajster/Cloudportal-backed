...
async function createBlueprintAwxEditor(deployment = {}, credentials = [], workflow = [], onChange = () => {}) {
...
async function proxmoxBlueprintForm(item = null, options = {}) {
  ...
  try {
    ...
    const awxEditor = await createBlueprintAwxEditor(
      deployment,
      credentials,
      item?.workflow || [],
    ...
    openModal({
      ...
      onSubmit: async (data, form) => {
        ...
        if (awx && existingAwxSteps.length && existingHasCloudInit) {
          workflow = existingWorkflow.map(step => step.type === 'register_awx'
            ? { ...step, retry: awxEditor.retry(), timeout: awxEditor.timeout() }
            : step);
        } else if (!awx && existingAwxSteps.length && existingHasCloudInit) {
          ...
          workflow = window.BlueprintFormUtils.blueprintWorkflow({
...
async function blueprintForm(item = null) {
  try {
    ...
    const deployment = item?.deployment || {};
    const awxEditor = await createBlueprintAwxEditor(deployment, credentials, item?.workflow || []);
