...
async function blueprintForm(item = null) {
  try {
    ...
    const defaultWorkflow = item?.workflow || (() => {
      ...
      const provider = templates.find(template => template.id === (item?.deployment?.template || 'proxmox-vm'))?.provider;
      return provider === 'proxmox'
        ? [apply, { id: 'vm_running', type: 'wait_for_vm', depends_on: ['apply'], retry: 0, timeout: 600, conditions: {}, rollback: null }, { id: 'guest_ip', type: 'wait_for_ip', depends_on: ['vm_running'], retry: 0, timeout: 600, conditions: {}, rollback: null }]
        : [apply];
    })();
