...
const CREDENTIAL_TYPE_CONFIG = {
  ...
  ssh: {
    ...
    authModes: {
      private_key: { label: 'Klucz prywatny (zalecane)', fields: [
        { key: 'private_key', label: 'Klucz prywatny SSH', tag: 'textarea', required: true, wide: true, placeholder: '-----BEGIN OPENSSH PRIVATE KEY-----', help: 'Wklej prywatny klucz OpenSSH/PEM. Klucz jest szyfrowany po stronie backendu.' },
        { key: 'known_hosts', label: 'known_hosts', tag: 'textarea', required: true, wide: true, placeholder: 'host.example.com ssh-ed25519 AAAA...', help: 'Wymagany wpis host key. Możesz przygotować go np. poleceniem ssh-keyscan -H host.' },
      ]},
      password: { label: 'Hasło', fields: [
        { key: 'password', label: 'Hasło SSH', type: 'password', required: true, autocomplete: 'new-password' },
        { key: 'known_hosts', label: 'known_hosts', tag: 'textarea', required: true, wide: true, placeholder: 'host.example.com ssh-ed25519 AAAA...', help: 'Wymagany wpis host key. Backend odrzuca nieznany klucz hosta.' },
      ]},
    },
  },
  winrm: {
    label: 'WinRM / Windows',
    description: 'Credential do zdalnego zarządzania Windows przez WinRM/NTLM.',
  ...
  aws: {
    ...
    description: 'Klucze IAM używane do uwierzytelnienia AWS. Test wykonuje STS GetCallerIdentity.',
    endpoint: null,
    username: null,
    tls: false,
    defaultAuth: 'keys',
    authModes: {
      keys: { label: 'Access keys', fields: [
        ...
        { key: 'session_token', label: 'Session Token (opcjonalnie)', tag: 'textarea', wide: true, help: 'Wymagany tylko dla tymczasowych poświadczeń STS.' },
      ]},
    },
  },
  azure: {
    label: 'Microsoft Azure',
    description: 'Service Principal / App Registration. Test wykonuje OAuth2 client_credentials wobec Microsoft Entra ID.',
    endpoint: null,
    username: null,
    tls: false,
    defaultAuth: 'service_principal',
    authModes: {
      service_principal: { label: 'Service Principal', fields: [
        { key: 'tenant_id', label: 'Tenant ID', required: true, placeholder: '00000000-0000-0000-0000-000000000000' },
        { key: 'client_id', label: 'Client ID', required: true, placeholder: '00000000-0000-0000-0000-000000000000' },
        { key: 'client_secret', label: 'Client Secret', type: 'password', required: true, autocomplete: 'new-password' },
        { key: 'subscription_id', label: 'Subscription ID (opcjonalnie)', placeholder: '00000000-0000-0000-0000-000000000000' },
      ]},
    },
  },
  openstack: {
    label: 'OpenStack',
    description: 'Uwierzytelnienie do Keystone v3 hasłem użytkownika i projektem.',
    ...
    defaultAuth: 'password',
    authModes: {
      password: { label: 'Password auth', fields: [
        { key: 'password', label: 'Hasło', type: 'password', required: true, autocomplete: 'new-password' },
        { key: 'project_name', label: 'Projekt', required: true, placeholder: 'admin' },
  ...
  other: {
    label: 'Inny sekret',
    description: 'Ogólny zaszyfrowany sekret. Ten typ nie ma wbudowanego adaptera testującego połączenie.',
...
function credentialSecretField(spec, enabled) {
  const wrapper = field(spec.label, 'secret_' + spec.key, {
    type: spec.type || 'text',
    tag: spec.tag,
    required: enabled && spec.required,
    value: enabled ? (spec.value || '') : '',
...
function renderCredentialDynamic(container, type, item) {
  ...
  if (config.username) {
    identity.append(field(config.username.label, 'username', {
      value: sameType ? item.username : '',
      required: config.username.required,
      placeholder: config.username.placeholder,
    }));
  }
  if (config.tls) {
    const tls = checkboxField('Weryfikuj certyfikat TLS', 'verify_ssl', sameType ? item.verify_ssl : true);
    tls.classList.add('credential-tls');
    identity.append(tls);
  }

  const typeCard = node('div', { class: 'credential-type-card' },
    node('div', {}, node('strong', { text: config.label }), node('span', { class: 'badge info', text: type })),
    node('p', { text: config.description }));

  const secretPanel = node('section', { class: 'credential-secret-panel' });
  secretPanel.append(node('div', { class: 'credential-secret-header' },
    node('div', {}, node('strong', { text: item ? 'Zapisany sekret' : 'Dane uwierzytelniające' }),
      node('p', { text: item ? 'Wartości sekretów nie są odczytywane z backendu. Pozostaw wymianę wyłączoną, aby zachować istniejący sekret.' : 'Pola poniżej są wysyłane wyłącznie przy zapisie i przechowywane zaszyfrowane.' })),
    badge(item?.configured ? 'configured' : 'new', item?.configured ? 'ok' : 'info')));

  let replaceInput = null;
  if (item) {
    const replace = checkboxField(mustReplace ? 'Zmiana typu wymaga zapisania nowych sekretów' : 'Zastąp zapisane sekrety', 'replace_secrets', mustReplace);
    replace.classList.add('credential-replace');
  ...
  authSelect.addEventListener('change', refreshSecrets);
  replaceInput?.addEventListener('change', refreshSecrets);
  refreshSecrets();
...
async function credentialsView() {
  ...
  dom.content.replaceChildren(heading('Sekrety infrastruktury są szyfrowane po stronie backendu i nigdy nie są zwracane do przeglądarki.', actions),
    table([
      { label: 'Nazwa', value: item => node('div', {}, node('strong', { text: item.name }), node('div', { class: 'muted', text: 'ID #' + item.id })) },
      ...
      { label: 'TLS', value: item => CREDENTIAL_TYPE_CONFIG[item.type]?.tls ? (item.verify_ssl ? badge('verify', 'ok') : badge('disabled', 'warning')) : badge('n/d') },
...
function credentialForm(item = null) {
  ...
  const fields = node('div', { class: 'form-grid' },
    field('Nazwa', 'name', { required: true, value: item?.name || '', maxlength: 100 }),
    typeField,
    dynamic);

  if (allowed('credentials.test')) {
    const testAfterSave = checkboxField('Po zapisaniu przetestuj połączenie', 'test_after_save', false);
    testAfterSave.classList.add('wide', 'credential-test-after');
    fields.append(testAfterSave);
  }
  ...
  openModal({
    ...
    eyebrow: 'Sekrety infrastruktury',
    body: fields,
    submitLabel: item ? 'Zapisz zmiany' : 'Dodaj credential',
    wide: true,
    onSubmit: async (data, form) => {
      ...
      const identityChanged = Boolean(item && (
        item.type !== type ||
        item.endpoint !== endpoint ||
        item.username !== username ||
        item.verify_ssl !== verifySsl
      ));

      if (identityChanged && !replaceSecrets) {
        throw new Error('Zmiana typu, endpointu, użytkownika lub ustawienia TLS wymaga zaznaczenia opcji zastąpienia sekretów.');
      }
      ...
      if (data.has('test_after_save') && type !== 'other') {
        try {
          const result = await api('/credentials/' + saved.id + '/test', { method: 'POST' });
          toast('Credential zapisany. Test połączenia zakończony powodzeniem' + (result.version ? ' (' + result.version + ')' : '') + '.');
        } catch (error) {
          toast('Credential został zapisany, ale test połączenia nie powiódł się: ' + error.message, 'error');
        }
      } else {
        toast('Credential zapisany.');
      }
      navigate('credentials');
    },
  });
