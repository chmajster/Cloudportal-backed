...
async function activateGlobalSearchResult(result) {
  ...
  if (result.entity?.type === 'vm' && typeof showVmDetailsPage === 'function' && allowed('vms.read')) {
    await showVmDetailsPage(result.entity.item);
  }
}
  ...
  try {
    ...
    const syncWorkflowGraph = () => {
      ...
      for (let level = 0; level <= maxDepth; level += 1) {
        ...
        const cards = levelSteps.map((step, stepIndex) => {
