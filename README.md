...
Szablon `proxmox-vm` klonuje **istniejący szablon QEMU cloud-init** przez bpg/proxmox 0.111.x. Wymaga storage, bridge, template_id i węzła; obsługuje DHCP albo statyczny IPv4 z IPAM. Template musi obsługiwać cloud-init. Blueprint może wskazać osobny credential SSH ustawiany na koncie wewnątrz VM: credential może zawierać hasło, klucz prywatny albo oba. Klucz prywatny nigdy nie jest kopiowany do VM — Cloudportal wykorzystuje wyłącznie wyliczony z niego klucz publiczny. Hasło jest odszyfrowywane dopiero przez worker podczas wykonania i przekazywane do Terraform przez `TF_VAR_ssh_password`, bez zapisywania go w Blueprint/deployment/job payload ani `terraform.tfvars.json`. Gdy Blueprint ma włączoną instalację QEMU Guest Agent, provisioning wybiera tryb automatycznie. Jeżeli storage `snippets` jest dostępny i worker ma poprawny dostęp SSH do noda Proxmox (`PROXMOX_VE_SSH_*` albo credential Proxmox oparty o użytkownika/hasło), używany jest szybki tryb vendor-data i `qemu-guest-agent` jest instalowany przez cloud-init. Gdy upload snippetu nie jest możliwy, workflow przełącza się na guest-bootstrap: generuje jednorazowy klucz Ed25519, uruchamia VM z tymczasowym kontem cloud-init i przez SSH instaluje oraz uruchamia `qemu-guest-agent`. Po uruchomieniu agenta konto docelowe z wybranego Credentiala jest tworzone przez Guest Exec API Proxmox, hasło jest ustawiane przez Guest Agent API, logowanie jest weryfikowane, a konto tymczasowe usuwane. Prywatny klucz bootstrapu istnieje wyłącznie w chronionym workspace (`0600`) i jest kasowany po pomyślnym bootstrapie. Przy DHCP fallback wymaga, aby adres dało się poznać przed instalacją agenta; dlatego dla template bez działającego QEMU Guest Agent należy użyć statycznego IP/IPAM. Token PVE nadal musi mieć wymagane uprawnienia do VM/storage/guest-agent.
