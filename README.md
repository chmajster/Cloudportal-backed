...
`.cpb` jest archiwum z wersjonowanym manifestem, dumpem PostgreSQL, bezsekretnym snapshotem ustawień runtime, trwałymi plikami `workspaces` (w tym lokalnym stanem Terraform i danymi recovery), SHA-256 każdego elementu i materiałem kryptograficznym wymaganym do migracji. Cache `.terraform`, locki wykonawcze i tymczasowe `terraform.tfvars.json` nie są przenoszone. Parser akceptuje wyłącznie zdefiniowaną whitelistę ścieżek, odrzuca linki, path traversal, nadmiarowe elementy, zły magic/type, brak manifestu, nieobsługiwany format i błędne checksumy.
...
- Testy credentiali: Proxmox, VMware, SSH hasłem lub kluczem prywatnym bez obowiązkowego `known_hosts`, WinRM z weryfikacją certyfikatu, AWS STS, Azure OAuth, OpenStack Keystone. Typ `other` przechowuje zaszyfrowane dane, ale wymaga osobnego adaptera testującego.
...
Szablon `proxmox-vm` klonuje **istniejący szablon QEMU cloud-init** przez bpg/proxmox 0.111.x. Wymaga storage, bridge, template_id i węzła; obsługuje DHCP albo statyczny IPv4 z IPAM. Template musi obsługiwać cloud-init. Blueprint może wskazać osobny credential SSH dla konta docelowego w VM. Gdy taki credential jest wybrany, wykonanie Blueprintu **nie korzysta z SSH do noda Proxmox ani z uploadu `snippets`**: Terraform tworzy jednorazowe konto `cpbootstrap...` z wygenerowanym kluczem Ed25519, a worker po apply łączy się tym kontem do gościa, tworzy konto docelowe i dopiero wtedy weryfikuje logowanie wybranym Credentialem. Jeżeli włączono instalację QEMU Guest Agent, ten sam bootstrap instaluje i uruchamia `qemu-guest-agent`, po czym konto docelowe i hasło mogą być finalizowane przez Guest Agent API oraz weryfikowany jest klucz hosta SSH. Bez instalacji QEMU Agent konto docelowe jest finalizowane bezpośrednio przez tymczasową sesję SSH. Konto bootstrapowe jest usuwane dopiero po poprawnej weryfikacji konta docelowego. Prywatny klucz bootstrapu istnieje wyłącznie w chronionym workspace (`0600`) i jest kasowany po sukcesie; hasło/klucz prywatny Credentiala docelowego nie trafiają do deployment/job payload ani `terraform.tfvars.json`. Gdy nie wybrano osobnego guest credentiala, instalacja QEMU Guest Agent nadal preferuje szybki tryb vendor-data przy dostępnym storage `snippets` i poprawnym SSH do noda Proxmox, a w przeciwnym razie używa guest-bootstrap. Przy DHCP bootstrap bez działającego wcześniej QEMU Guest Agent nadal nie może pewnie poznać adresu VM przed SSH, dlatego w takim przypadku należy użyć statycznego IP/IPAM.
...
Przepływ z opcją Ansible: init → plan → apply → adres VM z guest agent → wait_for_connection SSH/WinRM → zatwierdzony playbook → walidacja → successful. Bez Ansible kończy się po apply. Dla zwykłych credentiali SSH `known_hosts` nie jest wymagany; gdy go brak, połączenie Ansible nie wykonuje weryfikacji klucza hosta. Istniejące credentiale zawierające `known_hosts` nadal używają ścisłej weryfikacji. WinRM używa HTTPS i weryfikacji certyfikatu. Szablon musi zawierać odpowiednie konto dla playbooka. Bootstrap Linux wymaga bezhasłowego sudo tego konta.

API nie przyjmuje HCL, dowolnych ścieżek plików, pluginów, zmiennych `ansible_*` ani bezpośrednich komend executora. Playbooki systemowe pochodzą z repozytorium, a użytkownik z uprawnieniem `ansible.manage` może w **Katalog IaC → Playbooki Ansible** dodać własny wersjonowany playbook YAML, wkleić go lub wczytać z pliku `.yml/.yaml`, określić transport SSH/WinRM oraz jawne zmienne wejściowe z walidacją regex. Własny playbook musi używać `hosts: all`; backend odrzuca `delegate_to`, `local_action`, `connection`, controller-side lookup/query oraz moduły czytające/uruchamiające pliki po stronie kontrolera. Job zapisuje snapshot wersji własnego playbooka, więc późniejsza edycja katalogu nie zmienia już zakolejkowanego wykonania. OpenTofu używa wspólnego executora, jeżeli administrator zainstaluje `tofu` w `/usr/local/bin` lub `/usr/bin`.
