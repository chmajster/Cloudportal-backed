...
- Proxmox auto-token: formularz może przyjąć adres HTTPS, `user@realm` i hasło. POST `/api/v1/credentials/proxmox/auto-token` loguje się do PVE jednorazowo, tworzy losowo nazwany token `cloudportal-*` z `privsep=0`, weryfikuje go przez `/version` i zapisuje wyłącznie `token_id` + `token_secret`. Hasło nie trafia do tabeli credentiali ani odpowiedzi API. Operacja wymaga `Idempotency-Key`. Token dziedziczy uprawnienia użytkownika Proxmox, dlatego konto bootstrap powinno mieć minimalny wymagany zakres. Konta wymagające interaktywnego 2FA muszą użyć ręcznie utworzonego tokena.
