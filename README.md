...
Szablon `proxmox-vm` klonuje **istniejący szablon QEMU cloud-init** przez bpg/proxmox 0.111.x. Wymaga storage, bridge, template_id i węzła; obsługuje DHCP albo statyczny IPv4 z IPAM. Template musi obsługiwać cloud-init. Blueprint może wskazać osobny credential SSH ustawiany na koncie wewnątrz VM: credential może zawierać hasło, klucz prywatny albo oba. Klucz prywatny nigdy nie jest kopiowany do VM — Cloudportal wykorzystuje wyłącznie wyliczony z niego klucz publiczny. Hasło jest odszyfrowywane dopiero przez worker podczas wykonania i przekazywane do Terraform przez `TF_VAR_ssh_password`, bez zapisywania go w Blueprint/deployment/job payload ani `terraform.tfvars.json`. Gdy Blueprint ma wybrany credential SSH dla gościa, provisioning nie wymaga SSH do noda Proxmox. CloudPortal tworzy przez cloud-init losowe konto tymczasowe z jednorazowym kluczem Ed25519, po `terraform apply` łączy się nim z VM, tworzy konto docelowe z wybranego credentiala, opcjonalnie instaluje i uruchamia `qemu-guest-agent`, weryfikuje logowanie kontem docelowym i usuwa konto tymczasowe. Prywatny klucz bootstrapu jest przechowywany wyłącznie zaszyfrowany w payloadzie aktywnego joba i usuwany po powodzeniu. Dla tego trybu `cloud_init_snippet_storage` i `PROXMOX_VE_SSH_*` nie są wymagane. Legacy provisioning bez guest credentiala nadal używa snippet vendor-data i wymaga storage typu `snippets` oraz SSH do noda Proxmox. Przy DHCP bez działającego Guest Agenta adres VM musi być możliwy do ustalenia inną dostępną ścieżką; statyczny adres/IPAM nie ma tej zależności.
